...
AI-first 可辩护代码调查引擎（方向见 `docs/direction.md`）。Node >= 22.5，TypeScript 直跑（type stripping）。**Core 零模型调用；运行时依赖限审计过的白名单**（不联网、不越界读写、传递依赖可控、不破坏审计产物字节确定性；见 `docs/tool-selection.md` §一）。
