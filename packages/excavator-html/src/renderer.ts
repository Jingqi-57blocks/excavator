...
import { escapeHtml, markerMap, parseFrontMatter, renderMarkdown } from "./markdown.ts";
...
function renderPage(page: Page, pages: Page[], siteTitle: string): string {
  // The report content carries the target language; the `<html lang>` tag reflects it (English default).
  const language = page.metadata.language || "en";
  ...
  const moduleHub = page.outputName === "index.html" && featurePages.length
    ? `<section class="module-hub"><h2>${escapeHtml(UI.featureModules)}</h2><div class="module-grid">${featurePages.map((item) => `<a class="module-card" href="${escapeHtml(item.outputName)}"><strong>${escapeHtml(item.navTitle)}</strong><span>${escapeHtml(item.title)}</span></a>`).join("")}</div></section>`
    : "";
  const content = renderMarkdown(page.body.replace(/^#\s+.*$/m, "").trim(), markerMap(page.metadata));
  ...
  return `<!DOCTYPE html>\n<html lang="${escapeHtml(language)}"><head><meta charset="utf-8"/><meta content="width=device-width,initial-scale=1" name="viewport"/><title>${escapeHtml(page.title)}</title><link href="assets/report.css" rel="stylesheet"/></head><body>\n<header class="site-header"><div class="header-inner"><a class="brand" href="index.html">${escapeHtml(siteTitle)}</a><nav aria-label="${escapeHtml(UI.reportModules)}" class="global-nav">${nav}</nav></div></header>\n<div class="layout"><aside class="sidebar"><p class="sidebar-label">${escapeHtml(UI.onThisPage)}</p><nav aria-label="${escapeHtml(UI.tableOfContents)}" class="toc" id="toc"></nav></aside><main class="main"><section class="hero"><h1>${escapeHtml(page.title)}</h1></section>${moduleHub}<article class="content">${content}</article><div class="diagram-status">${escapeHtml(UI.diagramStatus)}</div></main></div>\n<footer class="footer">${snapshot}${runLine}</footer><button aria-label="${escapeHtml(UI.backToTop)}" class="back-to-top">↑</button><dialog class="diagram-dialog" id="diagramDialog"><div class="dialog-head"><strong>${escapeHtml(UI.diagramDetail)}</strong><button class="dialog-close">${escapeHtml(UI.close)}</button></div><div class="dialog-body"></div></dialog><script src="https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js"></script><script src="assets/report.js"></script></body></html>`;
}
...
/**
 * Rendered UI chrome (aria labels + visible strings) in one NEUTRAL English set. Chrome is not
 * per-language templated: the report CONTENT carries the target language, and evidence-chip text comes
 * from the report's own marker words (see markerMap). Localizing chrome, if ever wanted, is a
 * front-matter override — never a maintained per-language translation table here.
 */
const UI = {
  ...
  diagramStatus: "Diagrams could not be rendered. Reconnect and reload.",
