/**
 * Gate 1b, per authoring unit: every MATERIAL OBLIGATION this unit can reach must be grounded by THIS unit's own
 ...
 *
 * WHAT IT DOES NOT CLAIM. It does NOT say each obligation is disposed of exactly once across the document: primary
 * ownership is R5's, and asserting it here would be a promise this slice cannot keep. When two units of one
 * document name the same topic, EACH grounds its obligations in full — the duplication is visible in these
 * readings rather than resolved by silently letting one unit off. Nor does it read `detailBudget`: the epic already
 * ruled that a detail budget may not carry materiality or reachability semantics, so density is not this audit's
 * business.
 *
 ...
 *
 * THE OPEN-ORIGIN BUCKET IS THE FORK THIS SLICE CLOSES. The full audit's denominator excludes `origin === "open"`
 * work items; the catalog's bindings do not carry `origin` at all, and the plan accounting counts every material
 ...
 * requirement, never silently dropped from the reading.
 *
 * THREE STATES, NO BOOLEAN. `complete` (a non-empty set of OWED obligations, all of them grounded), `vacuous` (an
 * empty owed set — nothing reachable, or everything reachable exempt — carrying WHERE the emptiness comes from) and
 * `violations`. A synthesis names no topic and an
 * appendix may name only non-material ones, so `vacuous` is a normal outcome for them — and it must never render
 * with the same words as `complete`, which is the whole reason it is a separate state instead of "grounded 0 of 0".
 */
...
import type { MaterialObligationTopics } from "./plan-obligation-conservation.ts";
import { WORK_ITEM_STATUSES, type TopicObligationBinding } from "./topic-candidate.ts";

export const UNIT_GROUNDING_VERSION = "unit-grounding-v1";
...
/**
 * `denominator` is the GROUNDING denominator: reachable material obligations MINUS the open-origin exempt ones. A
 * `complete` arm whose denominator counted exemptions would print "grounds all N" over obligations it never
 * grounded, which is the kind of true-sounding sentence this codebase keeps paying for.
 */
export type UnitGroundingVerdict =
  | { readonly conclusion: "complete"; readonly denominator: number; readonly grounded: number; readonly openOriginExempt: number }
  | { readonly conclusion: "vacuous"; readonly denominator: 0; readonly openOriginExempt: number; readonly source: string }
  | {
...
export interface UnitGroundingResult {
  ...
  /** Every material obligation reachable through this unit's topics, ascending. The denominator. */
  readonly reachable: readonly string[];
  /** `reachable` minus the open-origin exemptions: what this unit's claims actually owe. */
...
export function auditUnitGrounding(input: UnitGroundingInput): UnitGroundingResult {
  ...
  for (const row of reach) {
    ...
    if (linked.length === 0) {
      ungrounded.push({
        ...
        problem: `material obligation ${JSON.stringify(row.workItemId)} (${row.dimension}) is represented by no claim of unit ${JSON.stringify(unit.unitId)}`
      });
      continue;
    }
    ...
    if (satisfying.length === 0) {
      ungrounded.push({
        ...
        dimension: row.dimension,
        status: binding.status,
        problem: `material obligation ${JSON.stringify(row.workItemId)} (${row.dimension}, ${binding.status}) has no ${groundingRequirementClause(binding.status)} in unit ${JSON.stringify(unit.unitId)}`
      });
      continue;
    }
    grounded.push({
  ...
  const groundingDenominator = reachable.length - openOriginExempt.length;
  const result: UnitGroundingResult = {
    ...
    kind: unit.kind,
    reachable,
    groundingDenominator,
    grounded,
    openOriginExempt,
    ungrounded,
    verdict: verdictOf(unit, groundingDenominator, grounded.length, openOriginExempt.length, ungrounded)
  };
  // The unit-level conservation law, asserted rather than trusted: every reachable obligation is in exactly one of
  // the three buckets. A residue would be an obligation this audit looked at and then forgot about.
  const summed = grounded.length + openOriginExempt.length + ungrounded.length;
  if (summed !== reachable.length) {
    throw new Error(`Unit ${JSON.stringify(unit.unitId)} grounding audit does not conserve: ${grounded.length} grounded + ${openOriginExempt.length} open-origin + ${ungrounded.length} ungrounded is not ${reachable.length} reachable material obligation(s)`);
  }
  return result;
}
 ...
 * hope. Both are fatal: an obligation whose row cannot be found is not an obligation that quietly needs no ground.
 */
 ...
 *
 * `vacuous` carries WHERE its empty denominator came from, and the sentence is chosen by unit kind — exhaustive, so
 * a fifth kind must say what an empty reachable set means for it before this compiles. A synthesis reaching nothing
 * is the design ("writes from child summaries only"); a leaf reaching nothing is a plan that gave it no material
 * topic, and those are not the same statement.
 */
function verdictOf(
  ...
  if (denominator === 0) return { conclusion: "vacuous", denominator: 0, openOriginExempt, source: vacuousSource(unit, openOriginExempt) };
  return { conclusion: "complete", denominator, grounded, openOriginExempt };
}

function vacuousSource(unit: PlanCatalogUnit, openOriginExempt: number): string {
...
export function summariseUnitGrounding(result: UnitGroundingResult): string {
  ...
  switch (verdict.conclusion) {
    case "complete":
      return `complete: unit ${result.unitId} grounds all ${verdict.denominator} material obligation(s) it owes (${verdict.openOriginExempt} open-origin exempt, ${result.reachable.length} reachable)`;
    case "vacuous":
      return `vacuous: unit ${result.unitId} owes no material obligation, so nothing was checked — ${verdict.source}`;
    case "violations":
      return `violations: unit ${result.unitId} leaves ${verdict.problems.length} of the ${verdict.denominator} material obligation(s) it owes ungrounded (${verdict.grounded} grounded, ${verdict.openOriginExempt} open-origin exempt)`;
  }
