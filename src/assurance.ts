 ...
 * adds the claim↔SEARCH-receipt support check (`auditClaimReceiptSupport`, Rule A). A run stamps this
 * at prepare (`manifest.assuranceVersion`); audit uses it to gate those strict checks: only runs
 * prepared under the current version are held to them, while older or field-less runs are
 * grandfathered so a later redaction/check bump never retroactively fails them.
 */
