...
**我自己的探针抓到的回归（送评后自查，must-fix 级）**：第一版按**算符**排除（把 `+=`/`:=` 整个踢出赋值路径），理由是「兜底会接住」。**兜底只接引号字面量**，于是无引号机密漏了：`API_TOKEN := sk-live-abc123`（Makefile）与 `apiKey += sk-live-abc123` **改动前会遮、改动后不遮**。这是本片唯一不许犯的错误方向。

**重新设计：按算符语义分，收窄放到值侧**。比较（`==`/`!=`/`<=`/`>=`/`=>`）出，**一切赋值（含 `+=`/`:=`/`??=`）留在赋值路径**；误遮由值侧否决解决——调用豁免 + **裸引用豁免（无数字的标识符）**。后者与文件里既有的 `isNameLikeLiteral` 同一条启发式（凭数字区分名字与密钥材料）。结果比第一版更好：`holiday.PtoToken = hours`（纯赋值）现在**也保住了**，那条我原本记为「已知代价」的被真正修好。
