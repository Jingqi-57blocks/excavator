...
## 建议语闸的否定判定：否定词侧是开放类，真正的修法是分词级识别（2026-08-16，57B-413 评审确认）

`recommendation-language.ts` 的**边界**侧是结构判定（经 658 份归档报告语料差分验证，旧新规则零差异），
但**否定词**侧仍是字符枚举，且**类是开放的**。实测漏放（真建议被豁免）：
`不少团队推荐采用该方案`、`无数案例的解决方案是`、`团队无疑推荐采用`、`不妨考虑解决方案 B`、
`无非是推荐采用`、`不外乎推荐采用`、`未尝不可推荐`。

**关键区分（评审补的，我自己没做）**：这些在**旧的裸词表规则下同样漏放**——是**存量限制，不是本次回归**。
本次净改进：`非常推荐` 被 `非(?!常)` 修掉；`不过，` / `无论如何，` / `不仅…，还…` 被逗号边界**结构性救回**。

**这条路补不完**（下一个是 `不禁`/`未免`）。真正的判据是**分词级否定词识别**，而那意味着引入中文分词
词库 = 依赖决策，须过审计白名单五条。**记账待裁，不继续加 lookahead。**

## 两条已接受的误拦（同上，语料零出现、失败方向可见）

- **冒号是双面的**：`报告不包含以下内容：修复建议、迁移步骤与优先级。` 被拦（旧规则放行，本次唯一
  前瞻性 FP 回归），而 `问题不在配置：解决方案是升级内核。` 确应被拦。区分需判断冒号前是否完整小句，
  超出正则。已钉成测试留裁决痕迹。
- **英文 `and` 作边界**：`does not include analysis and recommendations` 被拦（两宾语同属一个否定）。
  理由：英文否定辖域规范并列用 `or`（实测放行），且已钉的 `not shared and we recommend` 依赖 `and` 才能拦。
- 同类：成对破折号插入语 `本章不给出——也不暗示——修复建议。` 被拦，取舍同向。

## marker 词表三处定义已出现**内容级**分叉（2026-08-16，57B-413 评审指出，记账）

57B-413 把 `markersIn` 的中文词表扩到 8 个 token（加 `已验证`/`已推断`/`不可用`/`无法获得`），
而 `EVIDENCE_MARKER_TOKEN`（折叠用）与 `EVIDENCE_MARKER_WORD` 仍是 4 个。**这是三处定义
第一次出现内容级分叉**，此前只是语义不同、内容一致。

**后果（已核，非缺陷）**：新同义词不被 segmenter 剥离，于是 segment 会带着 `已验证` 进
claims scaffold 文本；覆盖判定是双向子串，故无硬误报；placement advisory 对新词静默。
**合并三处前必须先定「哪些语义差异是有意的」**（`markersIn` 接受英文裸词、TOKEN 只认反引号形态）。

## `markersIn` 改配对匹配后的一处新漏识别（2026-08-16，57B-413 评审指出，记账）

按整 token 匹配需要成对的反引号。散文里若有**落单**反引号，配对会位移，
`` `验证` `` 可能不被识别（旧的子串规则能认）。失败方向是**可见告警**（该章被报为无 marker）
而非静默，且输入本身是坏 markdown，故不修。
