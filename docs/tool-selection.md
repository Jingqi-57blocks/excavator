...
Core 仍**零模型调用**。放开的动机：CodeGraph 覆盖不到的语言（Perl/Zope 等）需要成熟解析库（`tree-sitter`）补全，自维护每语言解析器不可持续。放开**不改变**零依赖真正买到的两条——**可辩护、无黑箱**（依赖行为须可审计）与**供应链安全**（依赖会随引擎进入操作者指定的任何代码库，而**部署形态不固定**——本机跑与部署为服务都成立，所以不能靠「反正是本地」把这条抹掉）——白名单正是为守住这两条而设，不是放任任意依赖。周边包（`packages/*`）尽量轻量，可拥有自己的依赖。
