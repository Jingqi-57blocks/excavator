...
| **8 冻结** | 全部前层产物 + 各自完整度块 + `BoundRunContract` + 审计清单 | 封闭哈希链产物集，**按 epoch 封存**：completeness **按 CoverageDomain 逐域合取**，内嵌各台账摘要或其摘要+限定（声明「没看」禁止无限定地渲染「全齐」）；正 / 负关闭拆分；**每个 `NotApplicable` 的 determination 必被验证其 `coverageDigest` 所依赖的完整度**（扫描被 cap、读取失败、机制部分覆盖时 not-detected 不成立，降级为 Unavailable）；审计清单让「检查被跳过」本身可检查 | 冻结拒绝本身是一份带理由的 append-only 记录：不平衡 census、pending 声明、对照契约缺信封、determination 前提不成立 ⇒ **错误**；tier1 身份失配 ⇒ 劝告，tier2 失配 ⇒ 错误 | 记录产物集之外的任何东西；**只由 plan 计算的 completeness**（`src/assurance/freeze.ts:142-148` 的现状即禁例）；**对照当前代码的期望集而非本 run 契约验收信封**（追溯性失败的源头）；**跨 CoverageDomain 合成的单一完整度比值**；**原地改写已封存 epoch 的哈希链** | 全链摘要 + 契约摘要 + **epoch 序号**；归档 run 永远按其记录的契约验证——无契约的旧 run 按 `assuranceVersion` 世代闸（`src/assurance/assurance.ts:103`）复验，9 份已冻结 knowledge（v4×2 / v5×4 / v7×2 / v8×1）零迁移 |
...
**第 6/7 层的切法是从现状读出来的，不是排出来的。** 今天计划就在 prepare 铸出（`src/core/run.ts:495`），先于源码窗口阅读；而冻结后补证必须引用一个既有 work item（`run.ts:723-725`）——证据引义务、义务处置引证据，两个方向都真实存在。切成「声明在下（绝不引证据）、执行与处置同层在上」，环被声明侧的禁止输入列切断；调查在第 7 层内迭代生长，冻结是唯一封口。层序约束跨层依赖方向，不约束层内迭代。
...
**现状离这个序并不远。** 全量导入清点后，既有违序集中在六处；第七处是本方案自己的移动**将会创造**的，故一并列出：

| # | 违序边 | 解法（动哪些文件） |
|---|---|---|
| 1 | `src/snapshot/providers.ts:6` → `codegraph/module-detection`（边界层向上引事实层） | `module-detection.ts` **下移**到边界层（只依赖 node:fs / node:path，核实无障碍）——模块标记发现是 target 形状检测，不是图能力 |
| 2 | `src/codegraph/codegraph-command.ts:4` → `snapshot/providers`（取 `executableAvailable`） | `executableAvailable`（`src/snapshot/providers.ts:94`）**下移**到基座 |
| 3 | `src/context/boundary-functions.ts:26` → `assurance/decision-probe`（工作集层向上引探针） | `decision-probe.ts`、`condition-extract.ts`、`condition-extract-perl.ts` **下移**到事实层（探针是机制，不是义务） |
| 4 | `src/crossrepo/frontend-calls.ts:21`、`route-table.ts:25` → `assurance/condition-extract`（取 `loadAstGrep`） | 与 #3 同一次移动顺带解决：crossrepo 与 probe 同属第 3 层，平层引用合法 |
| 5 | `src/core/types.ts:1-4` → context 四个产物类型（基座向上引） | `PreparedContext`（`types.ts:487-512`，四个上引仅为它服务）**上移**到其生产者；`types.ts:1-4` 四行删除 |
| 6 | `src/core/run.ts`（1401 行，引 context/assurance 等 22 处）与 `types.ts` 同目录 | `run.ts`、`run-label.ts` **上移**到编排层；`src/core/` 只留基座 |
| 7 | **#6 落地即新生**：`src/assurance/parallel-authoring.ts:7` → run.ts（报告层向上引编排，取 `archiveCheckpoint`、`normalizeSection`） | 两函数（`run.ts:1542`、`:1470`，检查点机器，`run.ts:976-977` 自己也在用）**随 #6 同批下移**出 run.ts 到报告层的 checkpoint 模块 |

三个环因此消解：环一由 #1+#2 双向拆开；环二唯一的向上边是 #3，probe 下移后 `assurance/read-obligations.ts:27`、`relevance-annotation.ts:25` 对 context 的引用变为合法向下；环三由 #5+#6 把「同目录、依赖序两端」的假层拆成基座与编排——#7 是 #6 的必要配套，缺它则 #6 自己造出新违序。

**登记逐文件，没有默认层。** 「目录默认层 + 逐文件例外」与「未登记文件必须失败」不能同时严格成立——新文件会自动吃到默认层而不失败，盲区照旧。故登记表**只有显式条目**：目录条目仅当该目录整体属于一层时使用，`src/assurance/` 这类已知混装目录**不得有默认层**。第三节的目录结构给出的正是「让真实目录对应真实层」的落法，登记表因此退化为一份目录清单加少数过渡期例外。

**两处已知会「逼红」，红是正确输出：**

- `freeze.ts:2-3` 引 `assurance.ts`，而 `assurance.ts:18` 引报告侧 `claim-comparison.ts`——`assurance.ts`（1069 行）自身混装：工作项审计与版本闸是知识侧，claim / section 审计是报告侧。
- `assurance-artifacts.ts`（118 行）同样混装：`createAnalysisScope:17`、`emptyTraceCatalog:56`、`mergeTraces:60` 是调查侧，`collectClaims:85`、`writeReportCompanions:97` 是报告侧。整文件登记为报告层不准确。
...
一个目录一层（或一层的一个生产者），是登记表能退化成目录清单的前提。今天 `src/assurance/` 一个目录装了 19 个文件、横跨知识侧与报告侧，那既是层序盲区的来源，也是可读性问题。目标结构：

```
src/base/            共享类型、工具、注册表（语言 / 机制）、append-only 日志写入器
...
现有 19 份 `assurance/` 文件按此散开：探针三份（`decision-probe`、`condition-extract`、`condition-extract-perl`）→ `facts/probe/`；`read-obligations`、`relevance-annotation`、`logic-workitems` → `obligation/`；`read-coverage`、`read-residual-exposure`、`condition-inventory` → `investigation/`；`freeze` → `freeze/`；`timeline` → `base/`；报告侧六份（`authoring-packet`、`claims-scaffold`、`claim-comparison`、`parallel-authoring`、`section-slug`、`recommendation-language`）→ `report/`；`assurance.ts` 与 `assurance-artifacts.ts` 拆开后按侧分入。每个目录落在十个文件以内。
...
| P5 | census 分母取自索引 | 台账 → 工作集：census 分母只能是台账 RowSet；今天 `src/context/context.ts:36` 的 `censusRoots: GraphSummary["roots"]` 被第 5 层禁止输入列点名废止 |
...
| P17 | 事实包按文件边界收录 | **归属 → 工作集**：席位 `UnitId` 集是成员资格唯一权威；`context.ts:509` 传入而 `factpack.ts:247` 忽略的保留图，在新契约下先被第 4 层记录为 attribution。每个事实由第 3 层携带闭合联合的成员资格，第 5 层只按注册表声明的规则做 id 集合判定并标 retained / co-located / seeded，禁止重新按 path/span 猜测；「整文件作归属边界」被禁止输入列点名（实测 `handlers.go` 299 条 entrypoint 事实 293 条与功能无关且全部可引用，`factpack.ts:345-358`） |
...
2. **补一个工作集（消费）层。** `buildFactPack` 今天不属于任何层——生产与消费熔在一个函数里，契约没说保留图与整文件清单哪个权威，这正是 P17 长在那里的原因。
3. **义务拆为「声明」与「处置」，处置与证据、读取执行合为一个调查结果层。** 上一版把义务整体放在证据之上，但冻结后补证必须引既有 work item（`run.ts:723-725`）——证据引义务在旧序下是向上引用，契约自己违序；而计划本就在 prepare 铸出（`run.ts:495`），先于窗口阅读。声明在下（绝不引证据 id），执行与处置同层迭代生长，冻结按 epoch 封口。
