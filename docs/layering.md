...
**冻结的 epoch 模型（P0-4 的解法）。** 「不可变」与「允许冻结后补证」不能同时原地成立，而现行代码确实允许补证（`src/run/run.ts:896`：冻结后的写入必须携带一个能在 `workitems.json` 里解析到的 supplement）。裁定：**同世代内保留原地补证，但封存按 epoch 追加**——`freeze` 成功产出 `epoch N` 的封存清单；此后每次 supplement 记入 append-only 补证账，再次 `freeze` 产出 `epoch N+1`，前一个 epoch 的哈希链**只读不改**。报告绑定具体 epoch。这样：现有 supplement 机制不失效、而「补了什么」在产物上可见；跨到 v12 的旧 run 不改旧字节，重新 prepare。这个 epoch 模型不等于跨 schema 兼容读取。
...
**第 6/7 层的切法是从现状读出来的，不是排出来的。** 今天计划就在 prepare 铸出（`src/run/run.ts:616`），先于源码窗口阅读；而冻结后补证必须引用一个既有 work item（`src/run/run.ts:896`）——证据引义务、义务处置引证据，两个方向都真实存在。切成「声明在下（绝不引证据）、执行与处置同层在上」，环被声明侧的禁止输入列切断；调查在第 7 层内迭代生长，冻结是唯一封口。层序约束跨层依赖方向，不约束层内迭代。
...
| P14 | 未探测函数被静默丢弃 | 机制 → 义务声明：`mechanisms.json` 声明（语言 × 机制）缺口；`src/obligation/declarations.ts:80` 让每个候选进入 decision / exclusion / unavailable 之一；`src/assurance/read-obligations.ts:258` 的 legacy skip 不再是第 6 层声明边界 |
...
3. **义务拆为「声明」与「处置」，处置与证据、读取执行合为一个调查结果层。** 上一版把义务整体放在证据之上，但冻结后补证必须引既有 work item（`src/run/run.ts:896`）——证据引义务在旧序下是向上引用，契约自己违序；而计划本就在 prepare 铸出（`src/run/run.ts:616`），先于窗口阅读。声明在下（绝不引证据 id），执行与处置同层迭代生长，冻结按 epoch 封口。
