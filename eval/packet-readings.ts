...
//  2. Every byte of every packet lands in exactly one visible bucket. `work-item` / `evidence` / `trace` hold
//     the bytes attributable to a stable id; `anchor-line` holds an id's anchor line that is not part of its
//     full rendered chunk (the renderer's cross-block "see the X block" form, or an anchor whose chunk was
//     byte-bounded away); `unattributed` holds everything else — headings, the intro, the completeness header,
//     deterministic-fact listings, condition and reading-boundary blocks, and the joins between blocks.
//     `unattributed` is counted in the duplication denominator, never excluded to flatter the ratio. The
//     bucket sum is asserted equal to the packet's byte length, so there is no fourth state to hide in.
//
...
import type { AuditFinding, DocumentPlan, EvidenceItem, InvestigationPlan, RunManifest, SectionClaimsFile, TraceCatalog, TraceRecord } from "../src/base/types.ts";
import {
...
export type PacketUnitKind = "work-item" | "evidence" | "trace";
...
export const PACKET_UNIT_KINDS: PacketUnitKind[] = ["work-item", "evidence", "trace"];
...
export interface PacketByteBuckets {
  ...
  "anchor-line": number;
  /** Headings, intro, completeness, fact listings, condition/reading blocks, block joins — no stable id. */
  unattributed: number;
}
...
function projectDocument(
  ...
  const units = unitsOf(blocks, evidenceById, tracesById);
  const ranges: LocatedRange[] = [];
  const outcomes: Array<{ kind: PacketUnitKind; id: string; outcome: PacketUnitOutcome; bytes: number }> = [];

  for (const unit of units) {
    const chunk = Buffer.from(unit.chunk, "utf8");
    const chunkHits = findAll(packet, chunk);
    if (chunkHits.length > 1) fail(`document ${document.id}: the rendered chunk for ${unit.kind} ${unit.id} occurs ${chunkHits.length} times, so its bytes cannot be attributed`);
    const full = chunkHits.length === 1 ? { start: chunkHits[0], end: chunkHits[0] + chunk.length } : null;
    if (full) ranges.push({ ...full, kind: unit.kind, id: unit.id, anchorOnly: false });

    // Extra anchor lines: the renderer's cross-block "see the X block" form, or a tail the byte bound cut.
    let bytes = full ? full.end - full.start : 0;
    for (const lineStart of findAll(packet, Buffer.from(`\n${unit.anchor}`, "utf8")).map((index) => index + 1)) {
      if (full && lineStart >= full.start && lineStart < full.end) continue;
    ...
    const outcome: PacketUnitOutcome = full ? "full" : bytes > 0 ? "anchor-only" : "absent";
    outcomes.push({ kind: unit.kind, id: unit.id, outcome, bytes });
...
function bucketize(documentId: string, packetBytes: number, ranges: LocatedRange[]): PacketByteBuckets {
  ...
  const buckets: PacketByteBuckets = { "work-item": 0, evidence: 0, trace: 0, "anchor-line": 0, unattributed: 0 };
  ...
  const sum = buckets["work-item"] + buckets.evidence + buckets.trace + buckets["anchor-line"] + buckets.unattributed;
...
interface PacketUnit { kind: PacketUnitKind; id: string; chunk: string; anchor: string; }
 ...
 */
function unitsOf(blocks: PacketSection[], evidenceById: Map<string, EvidenceItem>, tracesById: Map<string, TraceRecord>): PacketUnit[] {
  const units: PacketUnit[] = [];
  const seen = new Set<string>();
  const push = (kind: PacketUnitKind, id: string, chunk: string): void => {
    const key = `${kind}:${id}`;
    if (seen.has(key)) return;
    seen.add(key);
    units.push({ kind, id, chunk, anchor: anchorOf(kind, id, chunk) });
  };
  for (const block of blocks) {
    for (const item of block.workItems) push("work-item", item.id, withoutHeading(renderWorkItems([item]), `work-item ${item.id}`));
    ...
    for (const item of block.workItems) {
      for (const traceId of item.traceIds ?? []) {
        ...
        push("trace", traceId, withoutHeading(rendered, `trace ${traceId}`));
      }
    }
  }
  return units;
