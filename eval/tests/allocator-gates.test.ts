...
function measure(caseId: string, options: Parameters<typeof allocateFeatureGraphRecorded>[5] = {}) {
  ...
  const allocation = allocateFeatureGraphRecorded(pool.nodes, pool.edges, pool.seeds, pool.anchorTerms, spec.budget.maxNodes, options);
...
test("M6: zero-signal and alias-deletion fixtures preserve the declared module census without forced seats", () => {
  for (const fixture of PREREG.gates.M6.fixtures) {
    ...
    const allocation = allocateFeatureGraphRecorded(pool.nodes, pool.edges, pool.seeds, pool.anchorTerms, 250);
